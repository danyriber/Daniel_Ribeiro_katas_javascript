...
const FamouStreamers = streamers.filter(
...
const streamersU = streamers2.filter(
...
const rareStreamers = streamers.filter(streamer => streamer.gameMorePlayed.includes('Legends')).map(streamer => {
    if (streamer.age > 35) {
      return {
        ...streamer,
